...
export function PrometheusCard({ tenantId, deploymentId }: PrometheusCardProps) {
  ...
  const loadConfig = async () => {
    try {
      setLoading(true);
      setError(null);
      setRevealedPassword(null); // Clear revealed password on reload
...
${labelsYaml}`;
  ...
  const handleReveal = async () => {
    ...
    try {
      ...
      showWarning(
        'Password Revealed',
        'Copy and save this password now. It won\'t be shown again until you rotate it.'
      );
  ...
  return (
    <>
      <div className="card">
        ...
        {/* Password revealed warning */}
        {revealedPassword && (
          <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-md">
            <p className="text-sm text-yellow-800 font-medium">
              ⚠️ Copy and save this password now. You won't see it again until you rotate it.
            </p>
          </div>
        )}
        ...
        {/* Revealed Password Display (if revealed) */}
        {revealedPassword && (
          <div className="mb-4">
            <label className="block text-sm font-medium text-gray-700 mb-2">
              Full Password (copy now):
            </label>
...
{buildYamlConfig(displayPassword)}
          </pre>
        </div>
